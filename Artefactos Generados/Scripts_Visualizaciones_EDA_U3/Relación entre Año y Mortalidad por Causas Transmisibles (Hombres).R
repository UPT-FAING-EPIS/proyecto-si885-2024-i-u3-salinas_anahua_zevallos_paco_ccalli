...
model_hombres <- lm(Mortalidad.por.Causas.Transmisibles..hombres ~ Year, data = dataset)
...
r_pearson_hombres <- cor(dataset$Year, dataset$Mortalidad.por.Causas.Transmisibles..hombres)
...
ggplot(dataset, aes(x = Year, y = Mortalidad.por.Causas.Transmisibles..hombres)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", color = "red") +
  labs(title = "Relación entre Año y Mortalidad por Causas Transmisibles (Hombres)", 
       x = "Año", 
       y = "Tasa de Mortalidad") +
  annotate("text", x = Inf, y = Inf, label = annotations_hombres, hjust = 1.1, vjust = 1.1, size = 3, color = "black") +
  theme_minimal() +
